...
public class LikeListAdapter extends RecyclerView.Adapter<LikeListAdapter.MyViewHolder> {
    ...
    public void onBindViewHolder(@NonNull LikeListAdapter.MyViewHolder holder, int position) {
        ...
        Picasso.with(context).load(likeOnPost.getActivityDetail().getIcon()).into(holder.ivTopIcon);
        //Picasso.with(context).load(imageUrl +likeOnPost.getActivityDetail().getImage()).into(holder.ivAllImage);/*.error(R.drawable.fq_bannnnnner)*/
        holder.tvBottomTittal.setText("" + likeOnPost.getActivityDetail().getDescription());
        String videoid = null;
        try {
            videoid = extractYoutubeId(likeOnPost.getActivityDetail().getVideo());
        ...
        if (!TextUtils.isEmpty(videoid)) {
            ...
            Picasso.with(context).load(img_url).error(R.drawable.back_icon).into(holder.ivAllImage);
           ...
           /* holder.ivAllImage.setOnClickListener(view -> {
                contentLikeCallback.contentLikeItem2(likeOnPost.getId());
            });*/
        } else {
